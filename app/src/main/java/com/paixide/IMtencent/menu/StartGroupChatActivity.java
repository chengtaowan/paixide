...
public class StartGroupChatActivity extends BaseActivity {
    ...
    private void init() {
        ...
        memberInfo.setAccount(userInfo.getName());
        memberInfo.setIconUrl(userInfo.getAvatar());
        ...
        mTitleBar.getRightTitle().setTextColor(getResources().getColor(R.color.read_dot_bg3));
        ...
        mContactListView.setOnSelectChangeListener(new ContactListView.OnSelectChangedListener() {
            ...
            public void onSelectChanged(ContactItemBean contact, boolean selected) {
                if (selected) {
                    //选中聊天对像
                    GroupMemberInfo memberInfo = new GroupMemberInfo();
                    memberInfo.setAccount(!TextUtils.isEmpty(contact.getNickname()) ? contact.getNickname() : contact.getId());
                    memberInfo.setIconUrl(TextUtils.isEmpty(contact.getAvatarurl()) ? "" : contact.getAvatarurl());
                    memberInfo.setSignature(contact.getRemark());
                    memberInfo.setUserid(contact.getId());
                    memberInfo.setNameCard(!TextUtils.isEmpty(contact.getNickname()) ? contact.getNickname() : contact.getId());
    ...
    private void createGroupChat() {
        ...
        for (int i = 1; i < mMembers.size(); i++) {
            groupName = (TextUtils.isEmpty(userInfo.getName()) ? groupName : userInfo.getName()) + "、" + mMembers.get(i).getAccount();
        }
