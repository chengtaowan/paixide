package com.paixide.IMtencent.menu;
...
//                menuActions.add(action);
                ...
                action.setIconResId(R.drawable.add_icon);
                action.setActionClickListener(popActionClickListener);
                menuActions.add(action);

                action = new PopMenuAction();
                action.setActionName(mActivity.getResources().getString(R.string.create_chat_room));
                action.setIconResId(R.drawable.build_group_icon);
                action.setActionClickListener(popActionClickListener);
                menuActions.add(action);
                mActions.clear();
                mActions.addAll(menuActions);
     ...
     * 监听回调事件
     */
    private PopActionClickListener popActionClickListener = new PopActionClickListener() {
        @Override
    ...
    public void show() {
        ...
        mMenuWindow.setBackgroundDrawable(mActivity.getResources().getDrawable(R.mipmap.top_pop));
    ...
    private void Namecenter() {
        Intent intent = new Intent();
        intent.setClass(mActivity, activity_Namecenter.class);
        mActivity.startActivity(intent);
    }
     ...
     * @return
     */
    private boolean chekstate() {
        switch (userInfo.getState()) {
            case 0:
            case 1:
                dialog_show2.mshow(mActivity, mActivity.getString(R.string.tv_msg_title) + "", mActivity.getString(R.string.tv_msg_msg1) + "", mActivity.getString(R.string.tv_msg_msg2) + "", callback);
                return true;
            case 3:
                dialog_Blocked.myshow(mActivity);
                return true;
            default:
                return false;
        }
